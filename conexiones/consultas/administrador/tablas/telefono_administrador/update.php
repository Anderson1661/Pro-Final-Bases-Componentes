<?php
// consultas/administrador/tablas/telefono_administrador/update.php
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $id_administrador = isset($input['id_administrador']) ? $input['id_administrador'] : '';
    ...
    if (empty($id_administrador) || empty($telefono_original) || empty($telefono_nuevo)) {
        echo json_encode(array("success" => "0", "mensaje" => "ID de administrador, teléfono original y teléfono nuevo son requeridos."));
        mysqli_close($link);
        exit;
    }
    ...
    // 1. Validar existencia del id_administrador (FK)
    $sql_check_admin = "SELECT 1 FROM administrador WHERE id_administrador = '$id_administrador'";
    $res_check_admin = mysqli_query($link, $sql_check_admin);
    
    if (mysqli_num_rows($res_check_admin) == 0) {
        echo json_encode(array("success" => "0", "mensaje" => "El ID de administrador ('$id_administrador') no existe."));
        mysqli_close($link);
        exit;
    }

    // 2. Realizar el DELETE del registro original
    $sql_delete = "DELETE FROM telefono_administrador 
                   WHERE id_administrador='$id_administrador' AND telefono='$telefono_original'";
    
    $res_delete = mysqli_query($link, $sql_delete);
    
    if ($res_delete) {
        if (mysqli_affected_rows($link) == 0) {
             echo json_encode(array("success" => "0", "mensaje" => "Error: No se encontró el teléfono original para el ID de administrador."));
             mysqli_close($link);
             exit;
        }
        
        // 3. Realizar el INSERT del nuevo registro
        $sql_insert = "INSERT INTO telefono_administrador (id_administrador, telefono) 
                       VALUES ('$id_administrador', '$telefono_nuevo')";
        
        $res_insert = mysqli_query($link, $sql_insert);
        
        if ($res_insert) {
            echo json_encode(array("success" => "1", "mensaje" => "Teléfono del administrador actualizado correctamente"));
        } else {
            // Revertir el DELETE si el INSERT falla (Ej: si el nuevo teléfono ya existe con ese admin)
            $sql_revert = "INSERT INTO telefono_administrador (id_administrador, telefono) 
                           VALUES ('$id_administrador', '$telefono_original')";

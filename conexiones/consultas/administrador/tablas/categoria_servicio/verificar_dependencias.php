...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($data['id_categoria_servicio'])) {
        $id_categoria_servicio = mysqli_real_escape_string($link, $data['id_categoria_servicio']);
        
        // Array para almacenar los resultados de las verificaciones
        $dependencies = array();
        
        // 1. Verificar en tabla ruta
        $query_rutas = "SELECT COUNT(*) as count FROM ruta WHERE id_categoria_servicio = '$id_categoria_servicio'";
        ...
        if ($total_dependencies > 0) {
            $response['success'] = "0";
            $response['mensaje'] = "No se puede eliminar la categoría de servicio porque tiene dependencias en otras tablas";
        ...
        $response['success'] = "0";
        $response['mensaje'] = "ID de categoría de servicio no proporcionado";
    }
} else {
    $response['success'] = "0";

...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($id_estado_servicio)) {
        ...
        $sql = "DELETE FROM estado_servicio WHERE id_estado_servicio='$id_estado_servicio'";
        $res = mysqli_query($link, $sql);
        
        if ($res) {
            if (mysqli_affected_rows($link) > 0) {
                echo json_encode(array("success" => "1", "mensaje" => "Estado de servicio eliminado correctamente"));
            } else {

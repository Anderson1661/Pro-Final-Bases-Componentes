...
$params[] = $id;
$query = "UPDATE cliente SET " . implode(', ', $updates) . " WHERE id_cliente = $" . $param_count;

$result = pg_query_params($conn, $query, $params);

if ($result) {
    if (pg_affected_rows($result) > 0) {
        send_response(200, ['success' => 'Cliente actualizado.']);
    } else {
        send_response(404, ['message' => 'No se encontró el cliente o no se realizaron cambios.']);
    }
} else {

package transportadora.Administrador.Tel_conductores // Asegúrate de que el paquete sea correcto
...
class Editar_tel_conductores: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        idconductorOriginal = intent.getIntExtra("id_conductor", 0)
        ...
        txtIdconductor.isEnabled = false

        // Cargar los datos
        if (idconductorOriginal != 0 && telefonoOriginal.isNotEmpty()) {
    ...
    private fun cargarTelefonoAdministrador() {
        val url = ApiConfig.BASE_URL + "consultas/administrador/tablas/telefono_conductor/consultar_tel_conductor.php"
        val jsonObject = JSONObject().apply {
            put("id_conductor", idconductorOriginal)
            ...
            { response ->
                try {
                    if (response.getString("success") == "1") {
                        ...
                        datosOriginales = TelefonoconductorData(
                            id_conductor = datos.getInt("id_conductor"),
                            telefono = datos.getString("telefono") // Teléfono original
                        )

                        // Rellenar campos: ID original y Teléfono para ser editado
                        txtIdconductor.setText(datosOriginales.id_conductor.toString())
    ...
    private fun guardarCambios() {
        ...
        val url = ApiConfig.BASE_URL + "consultas/administrador/tablas/telefono_conductor/update.php"
        ...
        val request = JsonObjectRequest(
            ...
            { response ->
                try {
                    Log.d("EditarTelconductor", "Respuesta actualización: $response")

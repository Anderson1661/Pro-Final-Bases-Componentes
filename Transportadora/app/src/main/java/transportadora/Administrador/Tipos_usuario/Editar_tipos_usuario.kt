...
class Editar_tipos_usuario : AppCompatActivity() {
    ...
    private lateinit var datosOriginales: TipoUsuario
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_editar_tipos_usuario)
        ...
        // Obtener el id_tipo_usuario pasado desde la actividad anterior
        idTipoUsuario = intent.getIntExtra("id_tipo_usuario", 0)
    ...
    private fun cargarTipoUsuario() {
        if (idTipoUsuario == 0) {
            Toast.makeText(this, "Error: ID de Tipo de Usuario no válido", Toast.LENGTH_SHORT).show()
            finish()
            return
        }

        // Ruta adaptada para el nuevo script
        val url = ApiConfig.BASE_URL + "consultas/administrador/tablas/tipo_usuario/consultar_tipo_usuario.php"
        ...
        val request = JsonObjectRequest(
            ...
            { response ->
                try {
                    Log.d("Editar_tipos_usuario", "Respuesta: $response")

                    if (response.getString("success") == "1") {
                        val datos = response.getJSONObject("datos")

                        // Extraer datos
                        val id = datos.getInt("id_tipo_usuario")
                        val descripcion = datos.getString("descripcion")

                        // Guardar datos originales
                        datosOriginales = TipoUsuario(id, descripcion)
                    ...
                    Log.e("Editar_tipos_usuario", "Error al procesar la respuesta: ${e.message}")
                    Toast.makeText(this, "Error al cargar los datos", Toast.LENGTH_SHORT).show()
                    finish()
                }
            },
            { error ->
                Log.e("Editar_tipos_usuario", "Volley error: ${error.message}")
    ...
    private fun guardarCambios() {
        ...
        if (!::datosOriginales.isInitialized) {
            Log.w("Editar_tipos_usuario", "Datos originales no inicializados. Forzando actualización.")
        }


        // Ruta adaptada para el nuevo script de actualización
        val url = ApiConfig.BASE_URL + "consultas/administrador/tablas/tipo_usuario/update.php"
        val jsonObject = JSONObject().apply {
            put("id_tipo_usuario", idTipoUsuario)
        ...
        val request = JsonObjectRequest(
            ...
            { response ->
                try {
                    Log.d("Editar_tipos_usuario", "Respuesta actualización: $response")

...
class Act_preguntas_administrador : AppCompatActivity() {
    ...
    // Lista de las respuestas y preguntas originales del admin
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cargarPreguntasRespuestasAdmin()
    ...
    private fun cargarTodasPreguntas() {
        CoroutineScope(Dispatchers.Main).launch {
            ...
            if (result.isNotEmpty()) {
                ...
                // Intenta actualizar la UI si las respuestas del admin ya están cargadas
                actualizarSpinnersYRespuestas()
            } else {
                Toast.makeText(this@Act_preguntas_administrador, "Error: No se pudo cargar el listado completo de preguntas.", Toast.LENGTH_LONG).show()
            }
        }
    }

    /** Carga las preguntas y respuestas actuales del admin */
    private fun cargarPreguntasRespuestasAdmin() {
        CoroutineScope(Dispatchers.Main).launch {
            ...
            if (success && data != null && data.preguntas.size == 3) {
                ...
                Toast.makeText(this@Act_preguntas_administrador, message, Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    private fun actualizarSpinnersYRespuestas() {
        ...
        for (i in 0 until 3) {
            ...
            // Buscar la posición de la pregunta actual del admin en la lista completa
    ...
    private fun guardarCambios() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            if (success) {
                val intent = Intent(this@Act_preguntas_administrador, Perfil_administrador::class.java)
    ...
    private fun mostrarDialogoDescartar() {
        ...
        builder.setPositiveButton("Sí") { dialog, _ ->
            dialog.dismiss()
            finish() // Cierra la actividad y vuelve a Perfil_admin
        }
    ...
    private fun obtenerTodasPreguntasAPI(): List<PreguntaCompleta> {
        val url = "${ApiConfig.BASE_URL}/consultas/administrador/perfil/consultar_preguntas_admin.php"
    ...
    private fun llamarAPIConsultarPreguntasRespuestas(correo: String): Triple<Boolean, PreguntasAdmin?, String> {
        val url = "${ApiConfig.BASE_URL}/consultas/administrador/perfil/consultar_respuestas_admin.php"
        ...
        return try {
            client.newCall(request).execute().use { response ->
                ...
                if (success && preguntasJson != null) {
                    ...
                    val data = PreguntasAdmin(idUsuario, preguntasList)
    ...
    private fun llamarAPIActualizarRespuestas(idUsuario: Int, datos: List<Triple<Int, Int, String>>): Pair<Boolean, String> {
        val url = "${ApiConfig.BASE_URL}/consultas/administrador/perfil/actualizar_respuestas_admin.php"

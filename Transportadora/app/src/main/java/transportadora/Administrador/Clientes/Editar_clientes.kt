...
class Editar_clientes : AppCompatActivity() {
    ...
    private lateinit var datosOriginales: Cliente
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(findViewById(R.id.editar_clientes)) { v, insets ->
            val systemBars = insets.getInsets(WindowInsetsCompat.Type.systemBars())
            v.setPadding(systemBars.left, systemBars.top, systemBars.right, systemBars.bottom)
            insets
        }

        // Obtener el id_cliente pasado desde la actividad anterior
    ...
    private fun cargarCliente() {
        ...
        val url = ApiConfig.BASE_URL + "consultas/administrador/tablas/cliente/consultar_cliente.php"
        ...
        val request = JsonObjectRequest(
            ...
            { response ->
                try {
                    Log.d("Editar_clientes", "Respuesta: $response")

                    if (response.getString("success") == "1") {
                        val datos = response.getJSONObject("datos")

                        // Extraer todos los datos
                        val id = datos.getInt("id_cliente")
                        ...
                        datosOriginales = Cliente(
                            id, identificacion, tipoIdentificacion, nombre,
                            direccion, correo, genero, paisNacionalidad, codigoPostal
                        )
                    ...
                    Log.e("Editar_clientes", "Error al procesar la respuesta: ${e.message}")
                    Toast.makeText(this, "Error al cargar los datos", Toast.LENGTH_SHORT).show()
                    finish()
                }
            },
            { error ->
                Log.e("Editar_clientes", "Volley error: ${error.message}")
    ...
    private fun guardarCambios() {
        ...
        if (paisNacionalidadStr.isEmpty()) {
            Toast.makeText(this, "El país de nacionalidad es requerido", Toast.LENGTH_SHORT).show()
            txtPaisNacionalidad.requestFocus()
            return
        }
        ...
        val url = ApiConfig.BASE_URL + "consultas/administrador/tablas/cliente/update.php"
        val jsonObject = JSONObject().apply {
            put("id_cliente", idCliente)
        ...
        val request = JsonObjectRequest(
            ...
            { response ->
                try {
                    Log.d("Editar_clientes", "Respuesta actualización: $response")
